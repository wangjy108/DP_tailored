...
class main():
    def __init__(self):
        ...
        if self.method:
            self._dic.setdefault("--method", self.method)
        
        
        try:
            self.rmsd_cutoff = sample["rmsd_cutoff"]
        except Exception as e:
            self.rmsd_cutoff = None
        
        if self.rmsd_cutoff:
            self._dic.setdefault("--rmsd_cutoff", self.rmsd_cutoff)
        
        try:
            self.energy_window = sample["energy_window"]
        except Exception as e:
            self.energy_window = None
        
        if self.energy_window:
            self._dic.setdefault("--energy_window", self.energy_window)
    ...
    def prepare(self):
        ...
        if self.input_sdf_db:
            self.main_dir = os.path.join(os.getcwd(), "SAMPLE")
        ...
        cmd_py = os.path.join(self.root_dir, 'binary_sample.py')
        
        if not os.path.isfile(cmd_py):
            logging.info(f"No binary_sample.py, please put it at {self.root_dir}")
            return 
        ...
        for ii, each_dir in enumerate(_work_dir):
            ...
            for each in get_set:
                ...
                cmd_line = f"python binary_sample.py --input_sdf {each} "

                for kk, vv in self._dic.items():
                    cmd_line += f"{kk} {vv} "
    ...
    def col_result(self):
        if not self.main_dir:
            _dir = os.path.join(self.root_dir, "SAMPLE")
            self.main_dir = _dir
        else:
            _dir = self.main_dir
